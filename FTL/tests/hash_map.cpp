...
int main( ) {
  ...
  double t{ 0. };
  {
    ...
    for( size_t i{ 0 }; i < 1000; ++i ) {
      if( hm.find( eng_keys[ i ] ) != hm.end( ) ) {
        ftl::println( "hash_map::find() test failed: erased value \"", eng_keys[ i ], "\" was found." );
      }
    }
    for( auto &it : hm ) {
      if( it.first.size( ) != it.second ) {
        ftl::println( "hash_map test failed: the KV pair {\"", it.first, "\", ", it.second, "}." );
      }
    }
  }
  ftl::println( "Took ", t, " seconds to run." );
  system( "pause" );
  return 0;
}
